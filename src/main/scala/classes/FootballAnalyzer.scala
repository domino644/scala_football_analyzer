...
class FootballAnalyzer (
  ...
  def getPlayerBlockCountAndRatio(): Option[Dataset[Row]] = {
    gameDF match {
      case Some(df) =>
  ...
  def getPlayerFoulsCommited(): Option[Dataset[Row]] = {
    gameDF match {
      case Some(df) =>
