...
class FootballAnalyzer(val spark: SparkSession) {
    ...
    def getPlayersWithNumbersAndPositions: DataFrame = {
        ...
        val selectedDF = gameDF.select(col("team.name").as("team_name"), explode($"tactics.lineup").as("lineup"))

        val playerDF = selectedDF.select(
            col("team_name"), col("lineup.player.name").as("player_name"),
            col("lineup.position.name").as("player_position"),
            col("lineup.jersey_number").as("player_number")
        ).distinct()

        val groupedDF = playerDF
            .groupBy(col("team_name"), col("player_name"), col("player_number"))
            .agg(collect_list($"player_position").as("player_positions"))
            .orderBy(col("team_name"), col("player_positions"))
    ...
    def getPlayerFoulsCommited: DataFrame = {
        ...
        var aggCommittedFoulExpression = Seq(
            ...
            count(when(col("foul_committed").isNull, 1)).as("standard_fouls_committed")
        )
        ...
        if (committedOffensiveFoulExist) {
            ...
            aggCommittedFoulExpression :+= count(when(col("foul_committed.advantage").isNotNull, 1)).as("advantage_fouls_committed")
        }

        if (committedPenaltyFoulExist) {
            aggCommittedFoulExpression :+= count(when(col("foul_committed.penalty").isNotNull, 1)).as("penalty_fouls_committed")
        }
        ...
        var withCommittedFoulsRatios = foulCommittedPerTypeCount
            .withColumn("standard_foul_committed_ratio", round(col("standard_fouls_committed") / col("total_fouls_committed") * 100, 2))
        ...
        if (committedAdvantageFoulExist) {
            ...
            withCommittedFoulsRatios = withCommittedFoulsRatios.withColumn("advantage_foul_committed_ratio", round(col("advantage_fouls_committed") / col("total_fouls_committed") * 100, 2))
        }

        if (committedPenaltyFoulExist) {

            withCommittedFoulsRatios = withCommittedFoulsRatios.withColumn("penalty_foul_committed_ratio", round(col("penalty_fouls_committed") / col("total_fouls_committed") * 100, 2))
        }
        ...
        val selectedCommittedFoulsColumns = Seq(
            ...
            col("standard_fouls_committed"),
            col("standard_foul_committed_ratio")
        ) ++ (
            ...
            if (diveFoulExist) Seq(col("dive_fouls"), col("dive_foul_ratio")) else Seq()
            ...
            if (committedAdvantageFoulExist) Seq(col("advantage_fouls_committed"), col("advantage_foul_committed_ratio")) else Seq()

            ) ++ (

            if (committedPenaltyFoulExist) Seq(col("penalty_fouls_committed"), col("penalty_foul_committed_ratio")) else Seq()
    ...
    def getPlayerFoulsWon: DataFrame = {
        ...
        var aggWonFoulExpression = Seq(
            ...
            count(when(col("foul_won").isNull, 1)).as("standard_fouls_won")
        )


        if (wonAdvantageFoulExist) {

            aggWonFoulExpression :+= count(when(col("foul_won.advantage").isNotNull, 1)).as("advantage_fouls_won")
        }
        ...
        if (wonPenaltyFoulExist) {
            ...
            aggWonFoulExpression :+= count(when(col("foul_won.penalty").isNotNull, 1)).as("penalty_fouls_won")
        }
        ...
        var withWonFoulsRatio = foulWonPerTypeCount
            .withColumn("standard_foul_won_ratio", round(col("standard_fouls_won") / col("total_fouls_won") * 100, 2))
        ...
        if (wonAdvantageFoulExist) {
            ...
            withWonFoulsRatio = withWonFoulsRatio.withColumn("advantage_foul_won_ratio", round(col("advantage_fouls_won") / col("total_fouls_won") * 100, 2))
        }


        if (wonPenaltyFoulExist) {

            withWonFoulsRatio = withWonFoulsRatio.withColumn("penalty_foul_won_ratio", round(col("penalty_fouls_won") / col("total_fouls_won") * 100, 2))
        }
        ...
        val selectedWonFoulsColumns = Seq(
            ...
            col("player_name"),
            col("total_fouls_won"),
            col("standard_fouls_won"),
            col("standard_foul_won_ratio")
        ) ++ (
            if (wonDefensiveFoulExist) Seq(col("defensive_fouls"), col("defensive_foul_ratio")) else Seq()

            ) ++ (

            if (wonAdvantageFoulExist) Seq(col("advantage_fouls_won"), col("advantage_foul_won_ratio")) else Seq()
            ) ++ (

            if (wonPenaltyFoulExist) Seq(col("penalty_fouls_won"), col("penalty_foul_won_ratio")) else Seq()
    ...
    def joinDF(join_elem:Seq[String], baseDF: DataFrame, dfs: DataFrame*): DataFrame = {
        ...
        val stringColumns = schema.fields.filter(_.dataType == StringType).map(_.name)

        val filledDF = joinedDF
            .na.fill(0, numericColumns)
            .na.fill("", stringColumns)
    ...
    def getTeamExactPassStats(teamName:String):DataFrame = {
        getExactPlayerPassInformation.where(col("team_name") === teamName)
    }


    def getTeamExactFoulsStats(teamName:String):DataFrame = {
        val foulsCommited = getPlayerFoulsCommited.where(col("team_name") === teamName)
        val foulsWon = getPlayerFoulsWon.where(col("team_name") === teamName)

        val totalFoulsStats = joinDF(Seq("team_name", "player_name"), foulsCommited, foulsWon)
        totalFoulsStats.show(truncate = false)
        totalFoulsStats
    }
